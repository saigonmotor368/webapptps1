...
// Trang chi tiết 1 đơn hàng của khách (dạng hóa đơn đầy đủ) — trước đây
// MyOrdersPage chỉ có 1 bảng dòng hàng thu gọn khi bấm mở rộng, thiếu hẳn
// thông tin giao hàng/thanh toán/người liên hệ và không tải được hóa đơn.
// Yêu cầu 2026-09-11: "ấn vào phải xem được thông tin chi tiết như 1 invoice
// + có nút tải hoá đơn khi đã hoàn thành". Dùng chung API /api/customer/orders
// (đã trả đủ field + invoice_document_id/confirmation_document_id) — không
// cần thêm API lấy 1 đơn riêng, đơn hàng của 1 khách hiếm khi nhiều tới mức
// cần phân trang.
export default function MyOrderDetailPage() {
  ...
  const fetchOrder = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`${apiBase}/api/customer/orders`, { headers: { Authorization: `Bearer ${token}` } });
  ...
  const downloadDocument = async (type: 'confirmation' | 'invoice') => {
    ...
    try {
      ...
      const res = await fetch(`${apiBase}/api/customer/${path}?orderId=${order.id}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      if (!res.ok) {
        const data = await res.json().catch(() => null);
        throw new Error(data?.error || 'Không tải được chứng từ');
      }
      ...
      alert('Lỗi: ' + (err.message || 'Không tải được chứng từ'));
    } finally {
      setDownloading(null);
    }
  };
  ...
  if (error || !order) return (
    <div className="space-y-4">
      <button onClick={() => navigate('/don-hang-cua-toi')} className="flex items-center gap-1.5 text-sm text-slate-500 hover:text-slate-700">
  ...
  const lineTotal = (item: any) => Number(item.finalLineTotal ?? item.lineTotal ?? (Number(item.finalUnitPrice ?? item.price) * Number(item.quantity)));

  return (
    <div className="space-y-6 pb-8">
      <header className="flex items-start justify-between gap-4 flex-wrap">
        <div className="flex items-center gap-3">
          <button onClick={() => navigate('/don-hang-cua-toi')} className="p-2 bg-white border border-slate-200 rounded-xl hover:bg-slate-50 text-slate-600 transition-colors">
            <ArrowLeft size={20} />
          </button>
          <div>
        ...
        <div className="flex gap-2 flex-wrap">
          {order.confirmation_document_id && (
            <button onClick={() => downloadDocument('confirmation')} disabled={downloading !== null}
          ...
          {order.invoice_document_id && (
            <button onClick={() => downloadDocument('invoice')} disabled={downloading !== null}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
                  ...
                  {items.map((item: any) => (
                    <tr key={item.id}>
                      <td className="px-4 py-3">
                        ...
                        {item.itemNote && <p className="text-xs text-slate-400 italic">{item.itemNote}</p>}
                      </td>
                      ...
                      <td className="px-4 py-3 text-right font-semibold text-slate-800">{money(lineTotal(item))}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
            ...
            {order.note && (
              <div className="p-4 border-t border-slate-100 bg-amber-50 text-amber-800 text-sm">
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-5 space-y-2 text-sm">
            <h2 className="font-bold text-slate-800 flex items-center gap-2 mb-2"><FileText size={18} className="text-green-600" />Tổng kết đơn</h2>
            <div className="flex justify-between text-slate-500"><span>Tạm tính</span><span className="font-medium text-slate-700">{money(order.subtotal)}</span></div>
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-5 space-y-3">
            <h2 className="font-bold text-slate-800 flex items-center gap-2"><MapPin size={18} className="text-green-600" />Giao hàng</h2>
              ...
              <dl className="space-y-2 text-sm">
                {(order.delivery_name || order.delivery_phone) && (
                  <div className="flex items-start gap-3">
                    <User size={16} className="text-slate-400 mt-0.5 shrink-0" />
                    <div>
                      <p className="text-slate-800">{order.delivery_name}</p>
                      {order.delivery_phone && <p className="text-xs text-slate-400">{order.delivery_phone}</p>}
                    </div>
                  </div>
                )}
